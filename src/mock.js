 ...
 */
module.exports = function(dirName, askForDeletion) {
    return function (done) {
        ...
        if(askForDeletion){
            promptOptions = promptOptions.concat(deletionOption);
        }
        ...
        inquirer.prompt(promptOptions, function (answers) {
            ...
            if(json.length !== undefined && json.length >= 0) {
                ...
                inquirer.prompt(promptOptionsId, function (ans) {
                    extractProperties (ans, answers, item);
                    makeMock(answers, dirName, askForDeletion, done);
                });
            };
...
function makeMock(answers, dirName, askForDeletion, done){
                ...
                .on('finish', function () {
                    if(!creation){
                        ...
                        gulp.src("./mock/mock.module.js")
                            ...
                            .pipe(gulp.dest("./mock"))
                            .on('finish', function () {
                                if (answers.delete && askForDeletion) {
                                    del("./" + answers.data);
                                }
                                if (askForDeletion) {
                                    done();
